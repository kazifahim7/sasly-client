...
import { updateProduct } from "@/service/listing";
...
export default function UpdateProduct({ product }: { product: ProductType }) {
    ...
    useEffect(() => {
        if (product) {
            ...
            setImage(product.images || ""); // Set existing image if available
        }
    }, [product, reset]);
    ...
    const onSubmit: SubmitHandler<FieldValues> = async (data) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                navigate.push("/dashboard/user/product")
                setImage('')
                ...
                <div className="p-8 mb-4 flex items-center flex-col gap-5 justify-center bg-gray-100 rounded-lg border border-gray-300">
                    {image === "" ? (
                        ...
                        <div className="relative w-full h-[200px] rounded-lg overflow-hidden shadow-md">
                            ...
                            <MdDelete className="text-2xl text-white bg-black bg-opacity-50 p-1 absolute top-2 right-2 cursor-pointer rounded-full hover:bg-opacity-75 transition" onClick={() => {
                                {
                                    setImage("")
                                    SetImageFile(undefined)
                                }
